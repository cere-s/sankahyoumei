...
import { blockUser } from '@/lib/interactions';
import { rateLimit, getClientIp } from '@/lib/rateLimit';

export async function POST(request: NextRequest) {
  if (!rateLimit(`interaction-block:${getClientIp(request)}`, 30, 60_000)) {
  ...
  const ok = await blockUser(user.id, blockedUserId, eventId);
  if (!ok) {
